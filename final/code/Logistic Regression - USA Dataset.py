...
USA_dataset.isnull().sum()

#Checking if predictor variable is binary or ordinal
sb.countplot(x='num', data = USA_dataset, palette='hls')
...
print (" Number of patients in dataframe: %i\n Number of patients with disease: %i\n Number of patients without disease: %i\n"
      % (len(USA_dataset.index),len(USA_dataset[USA_dataset.heart_disease==1].index),
         len(USA_dataset[USA_dataset.heart_disease==0].index)))
print (USA_dataset.head())
print (USA_dataset.describe())

from sklearn.metrics import accuracy_score
from sklearn.preprocessing import scale

#Xall = scale(data)
#Xall = USA_dataset.ix[3,:].values
#USA_data = USA_dataset.ix[:,(1,3)].values
USA_data = USA_dataset.ix[:,0:38].values
yall = USA_dataset.ix[:,39].values
#Xall = USA_dataset[new_columns_2[1:]].values

LogReg = LogisticRegression()
model = LogReg.fit(USA_data,yall)
print("Logistic Regression score on full data set:%f" %model.score(USA_data,yall))

y_pred = LogReg.predict(USA_data)
...
logit_model=sm.Logit(yall,USA_data)
...
Xsel0 = [patient for (patient,status) in zip(USA_data,yall) if status==0] # No-disease cases
Xsel1 = [patient for (patient,status) in zip(USA_data,yall) if status==1] # Disease cases
...
plt.suptitle("USA Data Set", fontsize=20)
